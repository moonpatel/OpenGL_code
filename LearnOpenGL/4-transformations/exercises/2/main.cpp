...
int main() {
    ...
    while(!glfwWindowShouldClose(mainWindow.win)) {
        ...
        trans2 = glm::scale(trans2,glm::vec3(sin(time),sin(time),0));
        shaderProgram.use();
        glUniformMatrix4fv(glGetUniformLocation(shaderProgram.ID,"transform"),1,GL_FALSE,glm::value_ptr(trans2));
        glDrawArrays(GL_TRIANGLES,0,3);
        shaderProgram.unuse();
